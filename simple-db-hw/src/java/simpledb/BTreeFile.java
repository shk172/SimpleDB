...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		//If it's not a leaf page, it is an internal page in this lab,
		//Fetch the page as internal page, and iterate through it.
		BTreeInternalPage _bPage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		Iterator<BTreeEntry> _bPageIterator = _bPage.iterator();
		BTreeLeafPage _leftChildPage = null;
		BTreeLeafPage _rightChildPage = null;
		
		
		//If f is null, recurse through the left pages and return the leftmost page
		if(f == null){
			_leftChildPage = findLeafPage(tid, dirtypages, _bPageIterator.next().getLeftChild(), perm, f);
		...
		_rightChildPage = findLeafPage(tid, dirtypages, nextEntry.getRightChild(), perm, f);
		return _rightChildPage;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		//No need to iterate through second half - adds to the runtime
		for (int i = 0; i < _numTuples / 2; i++) {
            if (_leafIterator.hasNext()) {
                Tuple _tupleToAdd = _leafIterator.next();
                page.deleteTuple(_tupleToAdd);
		...
		// update page's old left siblings' left sibling pointer
		BTreePageId _leftPid = page.getLeftSiblingId();
		if (_leftPid != null) {
			BTreeLeafPage _leftNeighbor = (BTreeLeafPage)getPage(tid, dirtypages, page.getLeftSiblingId(), Permissions.READ_WRITE);;
			_leftNeighbor.setRightSiblingId(_newLeftPage.getId());
			//dirtypages.put(rNeighbor.getId(), rNeighbor);
		}
		
		// link each page to the other
		_newLeftPage.setLeftSiblingId(page.getLeftSiblingId());
		_newLeftPage.setRightSiblingId(page.getId());
		page.setLeftSiblingId(_newLeftPage.getId());
		

		Field _nextField = _leafIterator.next().getField(this.keyField());
		
		// update pointers to parents
		BTreeInternalPage _newParentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), _nextField);
		_newLeftPage.setParentId(_newParentPage.getId());
		BTreeEntry _newEntry = new BTreeEntry(_nextField, _newLeftPage.getId(), page.getId());
		_newParentPage.insertEntry(_newEntry);
		 
		
		// return the appropriate value
		if (_nextField.compare(Op.GREATER_THAN_OR_EQ, field)) {
			return _newLeftPage;
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Field _fieldToPush = null;
		//prepare the loop
		int _numEntries = page.getNumEntries();
		Iterator<BTreeEntry> _entryIterator = page.iterator();
		
		//Put the first half into a new page - no need to iterate through the other
		//half unnecessarily
	      for (int j = 0; j < _numEntries / 2; j ++) {
              if (_entryIterator.hasNext()){
                  BTreeEntry e = _entryIterator.next();
                  page.deleteKeyAndLeftChild(e);
                  _newPage.insertEntry(e);
              }
              else{
            	  throw new DbException("There are no more entries");
              }
		...
		//Now that the entry pushed up has become the parent, we update the parent pointers of
		//the old page and the new page
		//I don't know how this function finds the correct page though..?
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, _newPage);
		
		
		//Make the new parent page for the parent entry
		BTreeInternalPage _newParent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), _fieldToPush);
		_newParent.insertEntry(_entryToPush);
		updateParentPointers(tid, dirtypages, _newParent);
		
		//return the appropriate page
		return (_fieldToPush.compare(Op.GREATER_THAN_OR_EQ, field)) ? _newPage : page;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//Initialize the iterator and the number of tuples to 'steal'
        int _numSteal = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        Iterator<Tuple> _siblingIterator;
        
        //If it's from the right sibling, start from the left (iterate normally through the lowest tuples)
        //If left, start in reverse to go through the highest values first
        if(isRightSibling == true){
        	_siblingIterator = sibling.iterator();
        }
        else{
        	_siblingIterator = sibling.reverseIterator();
        }
        
        Tuple _tupleToSteal = null;
        for (int j = 0; j < _numSteal; j ++) {
            if (!_siblingIterator.hasNext()) {
            	throw new DbException("There are no more tuples");
            }
            else{
            	_tupleToSteal = _siblingIterator.next();
                sibling.deleteTuple(_tupleToSteal);
                page.insertTuple(_tupleToSteal);
            }

        }
        // the stolen tuple now needs to update the parent
        if (isRightSibling) {
        	_tupleToSteal = _siblingIterator.next();
        }
        entry.setKey(_tupleToSteal.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//Initialize the same way as steal leaf function
        int _numSteal = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> _entryIterator = leftSibling.reverseIterator();

        BTreeEntry _centerEntry = new BTreeEntry(parentEntry.getKey(), null, page.iterator().next().getLeftChild());
        ...
        for (int j = 0; j < _numSteal; j ++) {
            ...
            BTreeEntry _leftEntry = _entryIterator.next();
            _centerEntry.setLeftChild(_leftEntry.getRightChild());
            page.insertEntry(_centerEntry);
            _centerEntry = new BTreeEntry(_leftEntry.getKey(), null, _leftEntry.getRightChild());
            leftSibling.deleteKeyAndRightChild(_leftEntry);
        }
        
        parentEntry.setKey(_centerEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//This function is implemented as a reverse to the stealfromleftinternalpage function
		
        int _numSteal = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> _entryIterator = rightSibling.iterator();

        BTreeEntry _centerEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), null);
        for (int j = 0; j < _numSteal; j ++) {
            if (!_entryIterator.hasNext()) {
            	throw new DbException("There are no more entries.");
            }
            
            BTreeEntry _rightEntry = _entryIterator.next();
            _centerEntry.setRightChild(_rightEntry.getLeftChild());
            page.insertEntry(_centerEntry);
            _centerEntry = new BTreeEntry(_rightEntry.getKey(), _rightEntry.getLeftChild(), null);
            rightSibling.deleteKeyAndLeftChild(_rightEntry);
        }
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreePageId _rightChild = leftPage.reverseIterator().next().getRightChild();
		BTreePageId _leftChild =  rightPage.iterator().next().getLeftChild();
        leftPage.insertEntry(new BTreeEntry(parentEntry.getKey(), _rightChild, _leftChild));
        
        Iterator<BTreeEntry> _entryIterator = rightPage.iterator();
